...
class bayesian_regression:
    ...
    def get_weight_posterior_paramters(self, X, y, sig_y=0.1, w_0=None, sig_w=1, sig_b=10):
        var_y = sig_y**2
        var_w = sig_w**2
        var_b = sig_b**2
        n, d = np.shape(X)
...
#        tmp[-1] = var_b
#        V_0 = np.diag(tmp)
        tmp = (1/var_w)*np.ones(d)
        tmp[-1] = (1/var_b)        
        Inv = np.diag(tmp)
        V_n = np.linalg.solve(var_y*Inv + np.dot(phi.T, phi), var_y*np.eye(d))
        if is_zero:
            w_n = (1/var_y)*np.dot(np.dot(V_n, phi.T), y)
        else:
            w_n = np.dot(np.dot(V_n, Inv), w_0.ravel()) + (1/var_y)*np.dot(np.dot(V_n, phi.T), y)           
        return w_n, V_n
    ...
    def train(self, X, y, w_0=None, hyperparams=[2],optimize=False):
        ...
        self.w_N, self.V_N = self.get_weight_posterior_paramters( X, y, sig_y=hyperparams[0], w_0=w_0, sig_w=hyperparams[1], sig_b =hyperparams[2] )
        if optimize:
           ...
           self.w_N, self.V_N = self.get_weight_posterior_paramters( X, y, sig_y=self.hyperparams[0], w_0=w_0, sig_w=self.hyperparams[1], sig_b =hyperparams[2])
    ...
    def get_error_bars(self, alpha,xs, w_N, V_N, sigma_y):
        ...
        for i in range(N):
            ...
            mu, var = self.get_prediction_parameters(phi, sigma_y, w_N, V_N)
            sigma = np.sqrt(var)
    ...
    def cost_func(self, params):
        ...
        try:
            self.w_N, self.V_N = self.get_weight_posterior_paramters( self.X, self.y, sig_y=params[0], w_0=self.w_0, sig_w=params[1], sig_b=params[2])
...
def br_plots(sig_y=2, sig_w = 100, X=None, y=None):
    ...
    w_N, V_N = br.train(phi_X, y, None, [4,1, 10], True)
    ...
    plt.figure()
    plt.title('Weight Posterior Distribution')
    tmp=np.sort(gaussian_random_samples(10000, np.mean(w_N), np.std(w_N)))
    plt.plot(tmp.ravel(), np.exp(log_gaussian_pdf(tmp,np.mean(w_N), np.std(w_N) )[0]))
    plot_2D_gaussian_scatter_PCA(10000, w_N, V_N)
    plt.legend(['PCA','MLE'])
